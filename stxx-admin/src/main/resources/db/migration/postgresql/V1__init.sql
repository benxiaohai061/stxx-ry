...
-- 模拟MySQL的FIND_IN_SET函数
-- FIND_IN_SET(needle, haystack) 返回needle在以逗号分隔的haystack中的位置(从1开始)，未找到返回0
-- 用法示例:
-- SELECT find_in_set('b', 'a,b,c'); -- 返回 2
-- SELECT find_in_set('d', 'a,b,c'); -- 返回 0
...
BEGIN
    ...
    -- 将haystack按逗号分割为数组
    parts := string_to_array(haystack, ',');
    ...
    FOR i IN 1..array_length(parts, 1) LOOP
        IF trim(parts[i]) = trim(needle) THEN
            RETURN i;
        END IF;
    END LOOP;
...
-- 完整版 STR_TO_DATE 函数
CREATE OR REPLACE FUNCTION str_to_date(p_text TEXT, p_format TEXT)
RETURNS TIMESTAMP AS $$
DECLARE
    v_format TEXT;
BEGIN
    -- MySQL -> PostgreSQL 格式映射
    v_format := p_format;

-- 年、月、日
v_format := replace(v_format, '%Y', 'YYYY');
v_format := replace(v_format, '%y', 'YY');
v_format := replace(v_format, '%m', 'MM');
v_format := replace(v_format, '%c', 'FMMonth'); -- 月份名称缩写可以自定义
v_format := replace(v_format, '%d', 'DD');
v_format := replace(v_format, '%e', 'FMDD');

-- 时分秒
v_format := replace(v_format, '%H', 'HH24');
v_format := replace(v_format, '%h', 'HH12');
v_format := replace(v_format, '%i', 'MI');
v_format := replace(v_format, '%s', 'SS');
v_format := replace(v_format, '%p', 'AM'); -- AM/PM

-- 星期相关（可扩展）
v_format := replace(v_format, '%W', 'Day');  -- 完整星期名称
v_format := replace(v_format, '%a', 'Dy');   -- 缩写星期名称

-- 返回 TIMESTAMP
RETURN to_timestamp(p_text, v_format);
